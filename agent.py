...
def clean_code(raw: str) -> str:
    """
    Extract Python code inside triple backticks if present,
    else return string stripped of stray backticks/whitespace.
    """
    raw = raw.strip()
    m = re.search(r"```(?:python)?\s*([\s\S]*?)```", raw, re.IGNORECASE)
    return m.group(1).strip() if m else raw.strip("` \n\r\t")

def generate_parser_code(pdf_summary: str, csv_sample: str,
                         feedback: str, model: str) -> str:
    prompt = f"""
Given a bank statement PDF with sample text:
{pdf_summary}
...
Clean currency symbols/commas and handle repeated headers,
empty rows and numeric conversion robustly.

Consider previous feedback/errors:
{feedback}
...
def agent_loop():
    ...
    for attempt in range(1, 4):
        ...
        try:
            # Generate parser code from API (stored in variable)
            parser_code = generate_parser_code(pdf_summary, csv_sample, feedback, model)

            # Write the generated code to file (no printing)
            write_parser(parser_code)

            # Test the parser by importing and running it
    ...
    # Use fallback parser if API-generated parser fails
